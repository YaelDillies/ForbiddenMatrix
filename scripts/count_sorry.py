...
def pretty_file(file: str):
    ...
    url = f"https://github.com/YaelDillies/ForbiddenMatrix/blob/master/{file}"
    return f"[{name}]({url})"

files = [file for tree in os.walk("ForbiddenMatrix") for file in glob(os.path.join(tree[0], '*.lean'))]
sorries = {}
